...
void Go(void)
{
    ...
    createParameters.myApplicationName = L"TGA 2D Tutorial 21";
...

    {
        ...
        Tga::Camera sideScrollingCamera;
        {
            ...
            sideScrollingCamera.SetRotation(Rotator(10, 0, 0));
        }
        ...
        Tga::Camera freeFlyCamera;
        {
            ...
            freeFlyCamera.SetRotation(Rotator(30, 0, 0));
        }
        ...
        ModelInstance platformModel = modelFactory.GetModelInstance(L"environment/platform_2.fbx");
        platformModel.SetTexture(0, 0, Tga::Engine::GetInstance()->GetTextureManager().GetTexture(L"environment/gradientMap_c.tga"));
        platformModel.SetScale({ 2.f, 2.f, 2.f });

        AnimatedModelInstance characterModel = modelFactory.GetAnimatedModelInstance(L"character/popp_sk.fbx");
        characterModel.SetTexture(0, 0, Tga::Engine::GetInstance()->GetTextureManager().GetTexture(L"character/atlas.tga"));

        ModelInstance cube = modelFactory.GetUnitCube();
        cube.SetTexture(0, 0, Tga::Engine::GetInstance()->GetTextureManager().GetTexture(L"cubetexture.tga"));

        AnimationPlayer idleAnimation = modelFactory.GetAnimationPlayer(L"character/idle.fbx", characterModel.GetModel());
        AnimationPlayer runAnimation = modelFactory.GetAnimationPlayer(L"character/run.fbx", characterModel.GetModel());
        AnimationPlayer jumpAnimation = modelFactory.GetAnimationPlayer(L"character/jump.fbx", characterModel.GetModel());
        ...
        tgeSpriteSharedData.myTexture = Tga::Engine::GetInstance()->GetTextureManager().GetTexture(L"sprites/tge_logo_w.dds");
        ...
        for (unsigned int i = 0; i < 20000; i++)
        {
            ...
            instance.myTransform = Matrix4x4f::CreateScaleMatrix(200.f);
        ...
        effectSpriteSharedData.myTexture = Tga::Engine::GetInstance()->GetTextureManager().GetTexture(L"sprites/Particle.dds");

        Tga::SpriteSharedData cloudSpriteSharedData;
        cloudSpriteSharedData.myTexture = Tga::Engine::GetInstance()->GetTextureManager().GetTexture(L"sprites/cloud.dds");

        Tga::Font font48 = Tga::Engine::GetInstance()->GetTextService().GetOrLoad(L"Text/arial.ttf", Tga::FontSize_48);
        Tga::Font font18 = Tga::Engine::GetInstance()->GetTextService().GetOrLoad(L"Text/arial.ttf", Tga::FontSize_18);
        ...
        while (bShouldRun)
        {
...

            {
                ...
                if (isFreeFly && GetForegroundWindow() == windowHandle)
                {
                    ...
                    {
                        camMovement += freeFlyCamera.GetTransform().GetMatrix().GetForward() * 1.0f;
                    }
                    ...
                    {
                        camMovement += freeFlyCamera.GetTransform().GetMatrix().GetRight() * -1.0f;
                    }
                    ...
                    freeFlyCamera.SetPosition(freeFlyCamera.GetTransform().GetPosition() + camMovement * camSpeed * timer.GetDeltaTime());
                    ...
                    freeFlyCamera.GetTransform().AddRotation(camRotation * camRotSpeed);
            ...
            // Handle character movement
            {
...

                {
                    ...
                    {
                        characterModel.SetScale(Vector3f(50.f, 50.f, -50.f));
                    ...
                    characterModel.SetLocation(playerData.playerPosition);         
                    ...
                    sideScrollingCamera.SetPosition(playerData.playerPosition + smoothCameraOffset + Vector3f(0.0f, 180.f, -500.0f));
                }
            ...
            // First draw everything using the 3d camera
            {
                ...
                // first draw opaque meshes and sprites
                {
                    ...
                    for (int i = -10; i < 10; i++)
                    {
                        platformModel.SetLocation({ (float)i * 400.f, -200, 0 });
...

                    {
                        ...
                        float size = 200.f;
                        Tga::Sprite3DInstanceData instance = {};
                        instance.myTransform = Matrix4x4f::CreateScaleMatrix(size);
                ...
                // after this, draw transparent object, which has to be ordered back to front
                {
                    ...
                    graphicsStateStack.SetDepthStencilState(DepthStencilState::ReadOnly);
                    ...
                    {
                        Tga::Sprite3DInstanceData instance = {};
                        instance.myTransform = Matrix4x4f::CreateScaleMatrix(20000.f);
...

                            {
                                ...
                                for (int i = 0; i < 5; i++)
                                {
...

                                    {
                                        ...
                                        Tga::Sprite3DInstanceData instance = {};
                                        instance.myTransform = Matrix4x4f::CreateScaleMatrix(10.f);
                                        instance.myTransform.SetPosition(position + Vector3f{ -0.5f * size, 0.5f * size, 0.f });
                    ...
                    {
                        cube.SetLocation({ -1600, 200.f, 100.f });
                        cube.SetRotation(cube.GetTransform().GetRotation() + Vector3f{ 45.f,45.f,0.f }*timer.GetDeltaTime());
