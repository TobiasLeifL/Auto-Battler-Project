	...
	Tga::LoadSettings(TGE_PROJECT_SETTINGS_FILE);
	{
		...
		while (true)
		{
			...
			// Arrow
			{
				...
				Tga::Engine::GetInstance()->GetDebugDrawer().DrawArrow(from, to, color, 100.f);
			}
			...
			// GetDebugDrawer will use this
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					Tga::Engine::GetInstance()->GetDebugDrawer().DrawLine({ 0, 0 }, { 200.f/ count, 0 }, { 1, 0, 0, 1 });
					Tga::Engine::GetInstance()->GetDebugDrawer().DrawLine({ 0, 0 }, { 0, 200.f / count }, { 0, 1, 0, 1 });
