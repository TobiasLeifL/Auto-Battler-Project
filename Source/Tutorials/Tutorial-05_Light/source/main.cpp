...
void Go( void )
{
	Tga::LoadSettings(TGE_PROJECT_SETTINGS_FILE);
	{
		...
		// Set normal map
		Tga::Texture* normalMap = engine.GetTextureManager().GetTexture(L"square_n.dds");
		spriteSharedData.myMaps[Tga::NORMAL_MAP] = normalMap;

		Tga::SpriteShader customShader; // Create
		customShader.Init(L"shaders/instanced_sprite_shader_vs.cso", L"shaders/PbrModelShaderPS.cso");
		...
		for (int i = 0; i < numberOfLogos; i++)
		{
			...
			positions.push_back({ randX , randY * engine.GetWindowRatioInversed() });
		}
		...
		{
			Tga::PointLight light(Tga::Transform(Tga::Vector3f(random0to1(), random0to1() * engine.GetWindowRatioInversed(), -0.1f)), Tga::Color(random0to1(), random0to1(), random0to1(), 1), 0.1f + (random0to1() + random0to1()), 0.2f);
