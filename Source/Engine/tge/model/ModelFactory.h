...
public:
	...
	AnimatedModelInstance GetAnimatedTgmModelInstance(const std::wstring& aTgmName);
	ModelInstance GetModelInstance(const std::wstring& someFilePath);
	ModelInstance GetTgmModelInstance(const std::wstring& aTgmName);
...
protected:
	...
	std::shared_ptr<Model> LoadModelW(const std::wstring& someFilePath);
